...
REPO_NAME = "mom"   # المستودع الرئيسي
...
# --- [GITHUB HELPERS (دفع إلى المستودع الرئيسي)] ---
def init_main_repo():
    """تهيئة المستودع الرئيسي للبوت (مرة واحدة)"""
    repo_path = os.getcwd()  # الدليل الحالي للبوت
...
def push_to_github(project_path, p_name):
    """
    رفع المشروع إلى المستودع الرئيسي تحت مجلد hosted_projects/{p_name}
    """
    ...
    try:
        # التأكد من وجود المجلد داخل hosted_projects
        rel_path = os.path.relpath(project_path, original_dir)  # مثلاً "hosted_projects/اسم_المشروع"
        if not rel_path.startswith("hosted_projects"):
            return False, "المشروع ليس داخل hosted_projects"

        # إضافة المجلد المحدد فقط
        ...
        # في حالة عدم وجود تغييرات (لا commit جديد) لا نعتبرها فشلاً
        if "nothing to commit" in e.stderr.decode():
            logger.info(f"لا توجد تغييرات جديدة للمشروع {p_name}")
            github_url = f"https://github.com/{GITHUB_USER}/{REPO_NAME}/tree/main/hosted_projects/{p_name}"
            return True, github_url
...
# --- [ɢɪᴛʜᴜʙ ꜱʏɴᴄ ꜱʏꜱᴛᴇᴍ - استرداد وتشغيل تلقائي] ---
async def sync_projects_from_github():
    """سحب المشاريع من المستودع الرئيسي (hosted_projects) وتشغيلها تلقائياً"""
    logger.info("🔄 جاري فحص المشاريع المخزنة على GitHub لاستعادتها...")
    # ننزل المستودع الرئيسي مؤقتاً لنرى محتويات hosted_projects
            ...
            for p_name in os.listdir(projects_dir):
                ...
                if os.path.isdir(p_path):
                    ...
                    if os.path.exists(main_file):
                        ...
                        logger.warning(f"⚠️ المشروع {p_name} لا يحتوي على main.py")
    except Exception as e:
...
def run_web():
    """تشغيل خادم Flask مع معالجة الأخطاء ومنع تعليق السيرفر"""
    try:
        # استخدام threaded=True للسماح بمعالجة متعددة الطلبات
        # وإضافة معالجة للاستثناءات لمنع تعليق السيرفر
        app.run(host='0.0.0.0', port=PORT, debug=False, threaded=True)
    except OSError as e:
        # خطأ المنفذ مشغول - محاولة استخدام منفذ بديل
        if "Address already in use" in str(e):
            logger.error(f"⚠️ المنفذ {PORT} مشغول! محاولة استخدام منفذ آخر...")
            try:
                # محاولة استخدام منفذ عشوائي
                app.run(host='0.0.0.0', port=0, debug=False, threaded=True)
            except Exception as e2:
                logger.error(f"❌ فشل تشغيل الخادم: {e2}")
        else:
            logger.error(f"❌ خطأ في تشغيل خادم Flask: {e}")
    except Exception as e:
        logger.error(f"❌ خطأ غير متوقع في خادم Flask: {e}")

# --- [ᴋᴇʏʙᴏᴀʀᴅ ꜱᴇᴛᴜᴘ - عربي كامل] ---
...
async def handle_text(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if user_id in user_upload_state and "path" in user_upload_state[user_id]:
        ...
        try:
            ...
            # رفع إلى GitHub (المستودع الرئيسي)
            ...
            if push_ok:
                ...
                await msg.edit_text(f"⚠️ **فشل رفع GitHub:** `{push_msg}`\nتم الاستضافة محلياً فقط.", parse_mode='Markdown')
                github_link = None
    ...
    # أوامر الأدمن فقط
        ...
        elif text == "🔓 فتح النظام":  # لن يظهر هذا الزر مباشرة لكن قد يكون موجودا في حالة التبديل
            bot_locked = False
            await animate(update.message.chat_id, None, context, Loading.executing(), delay=0.3, final_text="🔓 **تم فتح البوت.**")
            await update.message.reply_text("تم تحديث القائمة", reply_markup=get_main_keyboard(user_id), parse_mode='Markdown')

        # إعادة التشغيل التلقائي (تبديل)
        ...
        # مستخدم عادي ويوجد زر السجلات الحية
...
# --- [أوامر إضافية للأدمن عبر الكتابة (اختياري)] ---
...
def main():
    ...
    async def post_init(app):
        # 1. إضافة زر القائمة (الـ 3 شرطات) لسهولة الوصول
        from telegram import BotCommand
        await app.bot.set_my_commands([
            BotCommand("start", "🚀 تشغيل البوت والقائمة الرئيسية")
        ])
        
        # 2. مزامنة المشاريع من GitHub وتشغيلها تلقائياً عند بدء السيرفر (من المستودع الرئيسي)
        await sync_projects_from_github()
        
        # 3. تشغيل المهام الخلفية (السجلات، المراقبة، التنظيف)
